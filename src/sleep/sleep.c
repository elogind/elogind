...
static int nvidia_sleep(Manager* m, int fd, SleepOperation operation, unsigned* vtnr) {
        ...
        if (operation != _SLEEP_OPERATION_MAX) {
                *vtnr = 0;
                ...
                if (k < 0) {
                        log_debug_errno(k, "Failed to write '%s' to %s: %m", "resume", drv_suspend);
                }
...
static int execute(
        ...
        r = execute_directories(dirs, DEFAULT_TIMEOUT_USEC, gather_output, gather_args, (char **) arguments, NULL,
                                m->callback_must_succeed ? EXEC_DIR_NONE : EXEC_DIR_IGNORE_ERRORS);
