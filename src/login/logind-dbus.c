...
static int method_set_user_linger(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        path = strjoina("/var/lib/elogind/linger/", escaped);
...
static int method_send_prepare_for_sleep(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        r = send_prepare_for(m, handle_action_lookup(HANDLE_SUSPEND), active > 0);
        if (r < 0)
                return r;
...
static int elogind_run_helper( Manager* m, const char* helper, const char* arg_verb ) {
        ...
        r = execute_directories(dirs, DEFAULT_TIMEOUT_USEC, gather_output, gather_args, verb_args, NULL,
                                sleep_config->callback_must_succeed ? 0 : EXEC_DIR_IGNORE_ERRORS);
...
static const sd_bus_vtable manager_vtable[] = {
        ...
        /* Add a command to prepare all subscribers for the system going to sleep or to wake up */
