...
static const char* getenv_harder(pam_handle_t *handle, const char *key, const char *fallback) {
         ...
         * PAM services don't have to be reworked to set elogind-specific properties, but these properties
...
#if 0 /// Make it a background manager class if elogind is called by the openrc-user pam stack
        if (strchr(tty, ':')) {
#else // 0
...
_public_ PAM_EXTERN int pam_sm_close_session(
        ...
        if (!IN_SET(r, PAM_SUCCESS, PAM_NO_MODULE_DATA))
                return pam_syslog_pam_error(handle, LOG_ERR, r,
                                            "Failed to get PAM elogind.existing data: @PAMERR@");
