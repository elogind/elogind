...
int acquire_bus(BusFocus focus, sd_bus **ret) {
...
#if 0 /// With elogind arg_runtime_scope is always RUNTIME_SCOPE_SYSTEM
        ...
        if (!buses[focus]) {
                if (focus == BUS_MANAGER)
                        r = bus_connect_transport_elogind(arg_transport, arg_host, arg_runtime_scope, &buses[focus]);
                else
