...
int bus_set_address_machine(sd_bus *b, RuntimeScope runtime_scope, const char *machine) {
        ...
        if (rhs || runtime_scope == RUNTIME_SCOPE_USER) {
                 ...
                 * able to connect, we need to have a PAM session. Our way out?  We use elogind-run to get
                 * into the container and acquire a PAM session there, and then invoke elogind-stdio-bridge
                ...
                /* elogind-run -M… -PGq --wait -pUser=… -pPAMName=login elogind-stdio-bridge */

                a = strjoin("unixexec:path=elogind-run,"
                            "argv1=-M", erhs ?: ".host", ","
                            "argv2=-PGq,"
                            "argv3=--wait,"
                            "argv4=-pUser%3d", eu ?: "root", ",",
                            "argv5=-pPAMName%3dlogin,"
                            "argv6=elogind-stdio-bridge");
                if (!a)
                        return -ENOMEM;
