...
static void test_vtable(void) {
        ...
        assert(sd_bus_add_object_vtable(bus, NULL, "/foo", "org.freedesktop.elogind.testVtable", test_vtable_2, &c) >= 0);
        assert(sd_bus_add_object_vtable(bus, NULL, "/foo", "org.freedesktop.elogind.testVtable2", test_vtable_2, &c) >= 0);
        /* the cast on the line below is needed to test with the old version of the table */
        assert(sd_bus_add_object_vtable(bus, NULL, "/foo", "org.freedesktop.elogind.testVtable221",
                                        (const sd_bus_vtable *)vtable_format_221, &c) >= 0);

        assert(sd_bus_add_fallback_vtable(bus, NULL, "/fallback", "org.freedesktop.elogind.testVtable2", test_vtable_2, happy_finder, &c) >= 0);
