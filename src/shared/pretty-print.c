...
static int guess_type(const char **name, char ***ret_prefixes, bool *ret_is_collection, const char **ret_extension) {
        ...
        /* We assume elogind-style config files support the /usr-/run-/etc split and dropins. */
        ...
        if (path_equal(n, "udev/hwdb.d"))
                ...
                ext = ".install";
        else if (path_equal(n, "elogind/ntp-units.d")) {
                coll = true;
                ext = ".list";
        } else if (path_equal(n, "elogind/relabel-extra.d")) {
                coll = run = true;
                ext = ".relabel";
        } else if (PATH_IN_SET(n, "elogind/system-preset", "elogind/user-preset")) {
                coll = true;
                ext = ".preset";
        }
