...
static int on_properties_changed(sd_bus_message *m, void *userdata, sd_bus_error *error) {
        ...
        if (!streq(interface, "org.freedesktop.elogind1.Unit"))
                return 0;
...
int bus_wait_for_units_add_unit(
        ...
        *item = (WaitForItem) {
        ...
        r = sd_bus_call_method_async(
                        ...
                        item,
                        "s", FLAGS_SET(item->flags, BUS_WAIT_FOR_MAINTENANCE_END) ? NULL : "org.freedesktop.elogind1.Unit");
        if (r < 0)
