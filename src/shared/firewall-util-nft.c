...
#define NFT_SYSTEMD_TABLE_NAME   "io.elogind.nat"
...
int fw_nftables_add_masquerade(
         ...
         * elogind nat table.
         *
         * Unlike iptables -t nat -F (which will remove all rules added by the elogind iptables
...
static int fw_nftables_add_local_dnat_internal(
        ...
        if (r == -EOVERFLOW && af == AF_INET6) {
                /* The current implementation of DNAT in elogind requires kernel's
                 * fdb9c405e35bdc6e305b9b4e20ebc141ed14fc81 (v5.8), and the older kernel returns
                 * -EOVERFLOW. Let's treat the error as -EOPNOTSUPP. */
                log_debug_errno(r, "The current implementation of IPv6 DNAT in elogind requires kernel 5.8 or newer, ignoring: %m");
