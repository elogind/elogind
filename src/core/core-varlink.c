...
static int build_user_json(const char *user_name, uid_t uid, JsonVariant **ret) {
        ...
        return json_build(ret, JSON_BUILD_OBJECT(
                                   JSON_BUILD_PAIR("record", JSON_BUILD_OBJECT(
                                       ...
                                       JSON_BUILD_PAIR("service", JSON_BUILD_STRING("io.elogind.DynamicUser")),
...
static int vl_method_get_user_record(Varlink *link, JsonVariant *parameters, VarlinkMethodFlags flags, void *userdata) {
        ...
        if (!streq_ptr(p.service, "io.elogind.DynamicUser"))
                return varlink_error(link, "io.elogind.UserDatabase.BadService", NULL);
        ...
        if (uid_is_valid(p.uid))
                ...
                if (!v)
                        return varlink_error(link, "io.elogind.UserDatabase.NoRecordFound", NULL);
        ...
        if (!user_match_lookup_parameters(&p, un, uid))
                return varlink_error(link, "io.elogind.UserDatabase.ConflictingRecordFound", NULL);
...
int manager_varlink_init(Manager *m) {
        ...
        r = varlink_server_bind_method_many(
                        s,
                        "io.elogind.UserDatabase.GetUserRecord",  vl_method_get_user_record,
                        "io.elogind.UserDatabase.GetGroupRecord", vl_method_get_group_record,
                        "io.elogind.UserDatabase.GetMemberships", vl_method_get_memberships);
        if (r < 0)
