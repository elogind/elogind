 ...
 *    /sys/firmware/qemu_fw_cfg/by_name/opt/io.elogind.credentials/ is picked up and also placed in
 *    /run/credentials/@system/.
 *
 * 4. It imports credentials passed in via the DMI/SMBIOS OEM string tables, quite similar to fw_cfg. It
 *    looks for strings starting with "io.elogind.credential:" and "io.elogind.credential.binary:". Both
...
static int import_credentials_boot(void) {
        ...
        /* elogind-stub will wrap sidecar *.cred files from the UEFI kernel image directory into initrd
...
static int proc_cmdline_callback(const char *key, const char *value, void *data) {
        ...
        if (!proc_cmdline_key_streq(key, "elogind.set_credential"))
                return 0;
...
#define QEMU_FWCFG_PATH "/sys/firmware/qemu_fw_cfg/by_name/opt/io.elogind.credentials"
...
static int parse_smbios_strings(ImportCredentialContext *c, const char *data, size_t size) {
        ...
        for (p = data, left = size; left > 0; p += skip, left -= skip) {
                ...
                if ((n = memory_startswith(p, nul - p, "io.elogind.credential:")))
                        unbase64 = false;
                else if ((n = memory_startswith(p, nul - p, "io.elogind.credential.binary:")))
                        unbase64 = true;
                else {
...
int import_credentials(void) {
        ...
        if (envvar_set) {
                ...
                r = proc_cmdline_get_key("elogind.import_credentials", PROC_CMDLINE_STRIP_RD_PREFIX, &v);
                if (r < 0)
                        log_debug_errno(r, "Failed to check if 'elogind.import_credentials=' kernel command line option is set, ignoring: %m");
                else if (r > 0) {
                        r = parse_boolean(v);
                        if (r < 0)
                                log_debug_errno(r, "Failed to parse 'elogind.import_credentials=' parameter, ignoring: %m");
                        else if (r == 0) {
                                log_notice("elogind.import_credentials=no is set, skipping importing of credentials.");
                                return 0;
                        }
                }
