...
int mac_apparmor_setup(void) {
        ...
        if (!current_profile) {
                log_warning("Failed to get the current AppArmor profile of elogind from /proc/self/attr/apparmor/current or /proc/self/attr/current, ignoring.");
                return 0;
        }
        ...
        r = aa_change_profile("elogind");
        if (r < 0) {
                if (errno == ENOENT)
                        log_debug_errno(errno, "Failed to change to AppArmor profile 'elogind'. Please ensure that one of the binary profile files in policy cache directory %s contains a profile with that name.", cache_dir_path);
                else
                        log_error_errno(errno, "Failed to change to AppArmor profile 'elogind': %m");
                return 0;
        }

        log_info("Changed to AppArmor profile elogind.");
#endif
        return 0;
}
