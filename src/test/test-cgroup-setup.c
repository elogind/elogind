...
static void test_is_wanted(void) {
        assert_se(setenv("SYSTEMD_PROC_CMDLINE",
                         "elogind.unified_cgroup_hierarchy", 1) >= 0);
        test_is_wanted_print(false);

        assert_se(setenv("SYSTEMD_PROC_CMDLINE",
                         "elogind.unified_cgroup_hierarchy=0", 1) >= 0);
        test_is_wanted_print(false);

        assert_se(setenv("SYSTEMD_PROC_CMDLINE",
                         "elogind.unified_cgroup_hierarchy=0 "
                         "elogind.legacy_elogind_cgroup_controller", 1) >= 0);
        test_is_wanted_print(false);

        assert_se(setenv("SYSTEMD_PROC_CMDLINE",
                         "elogind.unified_cgroup_hierarchy=0 "
                         "elogind.legacy_elogind_cgroup_controller=0", 1) >= 0);
        test_is_wanted_print(false);
