...
#if 0 /// The number 88 is an evil code we do not want in elogind ; Use something nicer instead.
        r = sd_json_build(&ret, SD_JSON_BUILD_OBJECT(SD_JSON_BUILD_PAIR("yo", SD_JSON_BUILD_INTEGER(88))));
#else // 0
        r = sd_json_build(&ret, SD_JSON_BUILD_OBJECT(SD_JSON_BUILD_PAIR("yo", SD_JSON_BUILD_INTEGER(42))));
#endif // 0
        if (r < 0)
                return r;
