...
static ColorMode get_color_mode_impl(void) {
         ...
         * invoke elogind in a container or via a serial link or such, and use a true 256 color
         * terminal to do so. */
        return COLOR_256;
}
...
int pty_open_peer(int fd, int mode) {
        ...
        // TODO: Remove fallback path once baseline is updated to >= 4.13, i.e. elogind v258
